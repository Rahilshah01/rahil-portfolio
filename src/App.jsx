      ...
      <nav className="fixed top-0 w-full bg-slate-900/80 backdrop-blur-md border-b border-slate-700/50 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <button onClick={() => scrollToSection('about')} className="text-gray-300 hover:text-white transition">About</button>
        ...
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-cyan-400/30 bg-cyan-400/10 mb-6">
              ...
              <span className="text-cyan-400 text-sm">Available for hire</span>
            </div>
            ...
            <div className="flex gap-4">
              <button 
                ...
                className="px-6 py-3 border border-slate-600 rounded-lg font-semibold hover:border-slate-500 transition flex items-center gap-2"
              >
              <FolderDot size={20} />
                View Projects
              </button>
              <button className="px-6 py-3 border border-slate-600 rounded-lg font-semibold hover:border-slate-500 transition flex items-center gap-2"
      ...
      <section id="skills" className="py-20 px-6 bg-slate-800/30">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {skills.map((skill, idx) => (
              <div key={idx} className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 hover:border-cyan-400/50 transition">
                <div className="mb-4">{skill.icon}</div>
                <h3 className="text-xl font-bold mb-4">{skill.title}</h3>
                <div className="flex flex-wrap gap-2">
                  {skill.tags.map((tag, i) => (
                    <span key={i} className="px-3 py-1 bg-slate-700/50 text-sm rounded-full text-gray-300">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <section id="projects" className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-end justify-between mb-12">
            ...
            <button className="px-5 py-2 border border-slate-600 rounded-lg hover:border-slate-500 transition flex items-center gap-2"
              onClick={() => window.open('https://github.com/Rahilshah01', '_blank')}>
              View All on GitHub
            ...
            {projects.map((project, idx) => (
              <div key={idx} className="group bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden hover:border-cyan-400/50 transition">
                <div className="relative overflow-hidden">
                  <div className="absolute top-4 right-4 px-3 py-1 bg-slate-900/80 backdrop-blur-sm rounded-full text-xs font-semibold text-cyan-400 z-10">
                    {project.category}
                  </div>
                  <img 
                    src={project.image} 
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tags.map((tag, i) => (
                      <span key={i} className="px-2 py-1 bg-cyan-400/10 text-cyan-400 text-xs rounded font-mono">
                        {tag}
                      </span>
                    ))}
                  </div>
                  <div className="flex gap-3">
                    <a 
                      ...
                      className="flex-1 px-4 py-2 border border-slate-600 rounded hover:border-cyan-400 transition flex items-center justify-center gap-2 text-sm">
                      <Code size={16} />
                      Code
                    </a>
                    {project.liveLink ? (
                      <a 
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 px-4 py-2 bg-cyan-400/10 text-cyan-400 rounded hover:bg-cyan-400/20 transition flex items-center justify-center gap-2 text-sm">
                        <ExternalLink size={16} />
                        Live Demo
                      </a>
                    ) : (
      ...
      <section id="contact" className="py-20 px-6 bg-slate-800/30">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex flex-col items-center gap-4 p-6 bg-slate-800/50 border border-slate-700/50 rounded-xl hover:border-cyan-400/50 transition">
              ...
              <div className="text-center">
                ...
                <div className="font-semibold">rahilshah209.rs@gmail.com</div>
              </div>
            </div>

            <div className="flex flex-col items-center gap-4 p-6 bg-slate-800/50 border border-slate-700/50 rounded-xl hover:border-purple-400/50 transition">
              <div className="w-16 h-16 bg-purple-400/10 rounded-full flex items-center justify-center">
                <Phone className="text-purple-400" size={24} />
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-400 mb-1">Call Me</div>
                <div className="font-semibold">+1 (732) 522-7292</div>
              </div>
            </div>

            <div className="flex flex-col items-center gap-4 p-6 bg-slate-800/50 border border-slate-700/50 rounded-xl hover:border-blue-400/50 transition">
              <div className="w-16 h-16 bg-blue-400/10 rounded-full flex items-center justify-center">
                <MapPin className="text-blue-400" size={24} />
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-400 mb-1">Location</div>
                <div className="font-semibold">Jersey City, NJ</div>
              </div>
            </div>
          </div>
        </div>
